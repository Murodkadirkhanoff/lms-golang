...
func main() {
	cfg := config{
		port:    env.Int("AUTH_PORT", 4001),
		envName: env.String("LMS_ENV", "development"),
		db: database.Config{
			DSN:          env.String("AUTH_DB_DSN", "postgres://lms:devpassword@localhost:5432/lms_auth?sslmode=disable"),
		...
		jwtTTL:               env.Duration("JWT_TTL", 24*time.Hour),
	...
	logger := slog.New(slog.NewTextHandler(os.Stdout, nil)).With("service", "auth")
	...
	err = database.MigrateUp(db, env.String("AUTH_MIGRATIONS_PATH", "migrations"))
	if err != nil {
		logger.Error(err.Error())
		os.Exit(1)
	}
